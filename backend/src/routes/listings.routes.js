...
router.post('/', requireAuth, validateListingPayload, async (req, res) => {
  const db = await getDb();
  const {
    title,
    description,
...
router.post('/:id/purchase', requireAuth, async (req, res) => {
  ...
  const listing = await db.get('SELECT * FROM listings WHERE id = ? AND status = "approved" AND active = 1', req.params.id);
  if (!listing) return res.status(404).json({ error: 'Listing unavailable.' });

  const amount = mode === 'rent_day' ? listing.rent_day : mode === 'rent_hour' ? listing.rent_hour : listing.buy_price;
  ...
  const durationMs = mode === 'rent_hour' ? 60 * 60 * 1000 : mode === 'rent_day' ? 24 * 60 * 60 * 1000 : null;
